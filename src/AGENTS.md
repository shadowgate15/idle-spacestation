...
| Grouped stat layout (Overview) | `lib/components/ui/stat-panel/stat-panel.svelte` (33), `lib/components/ui/stat-row/stat-row.svelte` (231) | StatPanel = heading + `<dl class="grid grid-cols-[auto_1fr_auto]">`; StatRow uses `display: contents` so each row's 3 cells (label \| value+rate \| bar) participate in the parent grid. StatRow takes a discriminated-union `kind` prop: `scalar`/`stock`/`capacity`/`ratio`/`progress`/`label`. Replaces the old per-stat `StatTile` confusion (Power-as-a-sentence, Materials-meaning-two-things) on the Overview screen |
